...
DELETE FROM Habilidades_Personagem
WHERE id_personagem = 1 AND id_habilidade = 2;
...
SELECT
    ...
    COUNT(DISTINCT hp.id_habilidade) AS total_habilidades
FROM Personagem p
LEFT JOIN Personagem_Item pi ON pi.id_personagem = p.id
LEFT JOIN Habilidades_Personagem hp ON hp.id_personagem = p.id
GROUP BY p.id, p.nome, p.profissao, p.idade, p.id_mundo, p.data_criacao;
